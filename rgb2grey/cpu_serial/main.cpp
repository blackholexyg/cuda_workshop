...
void rgb2grey_cpu_serial(const uchar4 * const h_rgbaImage,
...
int main(int argc, char **argv) {
    ...
    // Converting -- CPU version
    start_time = std::clock();

    for(int ii=0; ii<100; ii++){
        rgb2grey_cpu_serial(h_rgbaImage, h_greyImage, numRows, numCols);
    }
    duration = ( std::clock() - start_time ) / (double) CLOCKS_PER_SEC;
    std::cout<<"Running time (CPU code): "<< duration << "s" << std::endl;

    // Output Image
    output_file = "cpu_serial.jpg";
